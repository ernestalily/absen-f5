...
<head>
    ...
    <title>Sistem Absensi Kelas Pro</title>
    <style>
...
<body>
        ...
        <div class="login-container">
            ...
            <form id="loginForm" onsubmit="handleLogin(event)">
                <div class="form-group">
                    ...
                    <input type="text" id="email" required placeholder="Masukkan email atau username">
                </div>
        ...
        <div class="container">
            ...
            <div class="header">
                <div class="header-left">
                    ...
                    <div class="user-info">Login sebagai: <span id="loggedUser"></span></div>
                </div>
            ...
            <div class="button-container">
                <button class="btn btn-save" onclick="saveAttendance()">💾 Simpan Absensi</button>
                <button class="btn btn-reset" onclick="resetToDefault()">🔄 Reset ke Hadir Semua</button>
                <button class="btn btn-export" onclick="exportToExcel()">📊 Export ke Excel</button>
                <button class="btn btn-history" onclick="showHistory()">📅 Lihat Riwayat</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const CREDENTIALS = {
            email: "takenbyf5@gmail.com",
            password: "SekretarisF5"
        };
        ...
        function generateStudentTable() {
            ...
            students.forEach((name, index) => {
                ...
                const status = attendanceData[name] || 'H'; // Default Hadir
                
                row.innerHTML = `
                    <td>${index + 1}</td>
                    <td>${name}</td>
                    <td>
                        <select class="status-select ${status.toLowerCase() === 'h' ? 'hadir' : status.toLowerCase() === 'i' ? 'izin' : status.toLowerCase() === 's' ? 'sakit' : 'alpha'}" 
                                data-student="${name}" 
        ...
        function updateStatus(select) {
            ...
            if (value === 'H') select.classList.add('hadir');
            else if (value === 'I') select.classList.add('izin');
            else if (value === 'S') select.classList.add('sakit');
            else if (value === 'A') select.classList.add('alpha');
        ...
        // ===== FUNGSI RESET KE DEFAULT (HADIR SEMUA) =====
        function resetToDefault() {
            if (confirm('Reset absensi hari ini ke HADIR SEMUA?')) {
                ...
                updateSummary();
                showToast('🔄 Semua siswa direset ke HADIR!', 'info');
            }
        }
        ...
        function exportToExcel() {
            ...
            if (!history) {
                showToast('⚠️ Belum ada data untuk di-export!', 'error');
                return;
            }
            ...
            students.forEach((student, index) => {
                ...
                dates.forEach(date => {
                    const status = historyData[date][student] || '-';
                    row.push(status);
                });
            ...
            // Download
            const fileName = `Absensi_Kelas_${getTodayDate()}.xlsx`;
                ...
                dates.forEach(date => {
                    ...
                    html += `
                        <div style="margin-bottom: 20px; padding: 20px; background: #f8f9fa; border-radius: 10px;">
                            <h3 style="color: #667eea; margin-bottom: 10px;">📅 ${date}</h3>
